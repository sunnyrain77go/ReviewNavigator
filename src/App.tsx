...
export default function App() {
  ...
  const handleAiPolish = async () => {
    ...
    try {
      ...
      const prompt = `
        你是一位專業的個人成長教練與寫作導航員，擅長將「吸引力法則」與「心理復盤」結合。
        使用者正在使用「${method}」復盤法進行記錄。
        請幫我將以下的原始回答整理成一篇結構清晰、語言流暢、充滿正向能量的「經驗卡片」。
        
        【你的任務】
        1. **行動感標題**：提煉一個具有「行動感」的標題（格式參考：「我如何…」或「X 件事讓我…」）。
        2. **正向敘事（100–300字）**：以第一人稱整理。請在敘述中保留我的真實感受與洞察並自然融入「正向情感字句」（例如：我真的覺得很幸運、這件事太棒了、我感到內心充滿力量等），將原始內容轉化為一段感恩且流暢的故事。
        3. **核心洞察**：從內容中找出一個關於成長或心智的體悟。
        4. **價值宣言**：請以「我值得...的人生」為開頭，結合今日內容，寫下一句強而有力的自我肯定句。
        5. **行動建議**：提供 1-2 個具體、可執行的下一步。
        6. **角色意義**：用一句話總結這個經驗對我的人生角色有什麼意義。
        7. **附註原始內容**：置於最下方，並在標題後方加上今天的日期（格式：YYYY-MM-DD）。
  ...
  const generatedPrompt = useMemo(() => {
    ...
    return `你是一位專業的個人成長教練與寫作導航員，擅長將「吸引力法則」與「心理復盤」結合。
使用者正在使用「${method}」復盤法進行記錄。
請幫我將以下的原始回答整理成一篇結構清晰、語言流暢、充滿正向能量的「經驗卡片」。

【寫作風格要求】
- 融入《睡前3分鐘超感謝日記》的精神，加入「我真是太幸運了」、「太感激了」等情感字句。
- 語氣真誠，像是在跟最親近的朋友分享好消息。
